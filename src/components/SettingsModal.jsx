...
function ApiCard({ cat, api, current, advanced, onPatch, onRemove, onDuplicate, onSetCurrent, onSelectModel }) {
  ...
  const test = async () => {
    ...
    const inputs = cat === 'reverse'
      ...
      : cat === 'agent'
        ...
        : { prompt: 'ping' }
    onPatch({ testMsg: undefined, testStatus: undefined })
    onPatch({ _busyTest: true })
    try {
      const data = await apiRun(cfg, inputs)
      const v = (data.output?.value || '').slice(0, 40)
      onPatch({ testMsg: t('settings.testOk', { msg: v || 'OK' }), testStatus: 'ok' })
    } catch (err) {
      onPatch({ testMsg: t('settings.testFail', { msg: err.message || err }), testStatus: 'err' })
    } finally {
  ...
  const fetchModels = async () => {
    if (!api.baseUrl) {
      onPatch({ testMsg: t('settings.needBaseUrl'), testStatus: 'err' })
      return
    }
    onPatch({ _busyFetch: true, testMsg: t('settings.fetching'), testStatus: undefined })
    try {
      const data = await apiListModels({ baseUrl: api.baseUrl, apiKey: api.apiKey, path: '/models' })
      onPatch({ models: data.models || [], testMsg: t('settings.fetchOk', { count: (data.models || []).length }), testStatus: 'ok' })
    } catch (err) {
      onPatch({ testMsg: t('settings.fetchFail', { msg: err.message || err }), testStatus: 'err' })
    } finally {
  ...
  return (
    <div className={`api-card ${current ? 'api-card-current' : ''}`}>
      ...
      <PField label={t('settings.field.model')} hint={t('settings.field.modelHint', { baseUrl: api.baseUrl || '{BaseURL}' })}>
        ...
        {api.testMsg && <div className={`api-test-msg ${api.testStatus === 'err' ? 'err' : 'ok'}`}>{api.testMsg}</div>}
      </PField>
